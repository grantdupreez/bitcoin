...
btc_df['bollinger_signal'] = np.where((int(btc_df['bollinger_long']) + int(btc_df['bollinger_short'])) > 0), btc_df['Close'], None)
...
fig = go.Figure(data=[go.Candlestick(x=btc_df['Datetime'],
            ...
            close=btc_df['Close']), 
              ...
              go.Scatter(x=btc_df.Datetime, y=btc_df.bollinger_signal, line=dict(color='black', width=1), name='Signal')
